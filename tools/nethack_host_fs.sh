#!/bin/sh
# Host build WITH the TCMIPS RAM filesystem layer active (TCM_FS_HOST).
# Level-file lifecycle bugs reproduce locally at full speed.
set -e
cd "$(dirname "$0")/.."
SRC=src/games/nethack
OUT=/tmp/opencode/nhhost_fs
mkdir -p $OUT
SRCS=""
for f in $SRC/core/*.c $SRC/tty/*.c $SRC/sys/*.c $SRC/lua/*.c \
         $SRC/tcm_main.c $SRC/tcm_vterm.c $SRC/tcm_termcap.c \
         $SRC/tcm_kbd.c $SRC/tcm_dirent.c $SRC/nhfont.c $SRC/tcm_hooks.c \
         $SRC/tcm_fs.c $SRC/tcm_console_sim.c; do
    case "$f" in
        */lua/lua.c|*/lua/luac.c|*/lua/onelua.c) continue ;;
    esac
    SRCS="$SRCS $f"
done
gcc -O1 -g -std=gnu99 -DTCM_HOST -DTCMIPS_PORT -DTCM_FS_HOST -DNO_SIGNAL -DNOMAIL \
