...
# cmake/ninja: system PATH first; override CMAKE=... if using a bundled copy
CLION_BIN="$HOME/.local/share/JetBrains/Toolbox/apps/clion/bin"
[ -x "$CLION_BIN/cmake/linux/x64/bin/cmake" ] && export CMAKE=${CMAKE:-$CLION_BIN/cmake/linux/x64/bin/cmake}
[ -d "$CLION_BIN/ninja/linux/x64" ] && export PATH=$CLION_BIN/ninja/linux/x64:$PATH
export CMAKE=${CMAKE:-cmake}
$CMAKE --build cmake-build-release --target tcmips_nethack -j8 > /tmp/nh_build.log 2>&1
