...
{
    tcm_flseek(grhandle,GRFILEPOS(chunk),SEEK_SET);
    tcm_fread(grhandle,&chunkexplen,sizeof(chunkexplen));
...
{
    (void)filename; (void)ptr; (void)length;
    return false;
}
...
{
    int handle = tcm_fopen(filename);
    if (handle == -1)
        return false;

    int32_t size = (int32_t)tcm_fsize(handle);
    *ptr=malloc(size);
    CHECKMALLOCRESULT(*ptr);
    if (!tcm_fread (handle,*ptr,size))
    {
        tcm_fclose (handle);
        return false;
    }
...
//
    ...
    handle = tcm_fopen(fname);
    if (handle == -1)
        CA_CannotOpen(fname);

    tcm_fread(handle, grhuffman, sizeof(grhuffman));
    tcm_fclose(handle);
    ...
    long headersize = tcm_flseek(handle, 0, SEEK_END);
    tcm_flseek(handle, 0, SEEK_SET);
    ...
    tcm_fread(handle, data, sizeof(data));
    tcm_fclose(handle);
...
//
    ...
    tcm_fread (grhandle,compseg,chunkcomplen);
...
//
    for (i=0;i<NUMMAPS;i++)
    {
        ...
        tcm_flseek(maphandle,pos,SEEK_SET);
        tcm_fread (maphandle,(memptr)mapheaderseg[i],sizeof(maptype));
    }
...
void CA_Shutdown (void)
{
    ...
    if(maphandle != -1)
        tcm_fclose(maphandle);
    if(grhandle != -1)
        tcm_fclose(grhandle);
    if(audiohandle != -1)
        tcm_fclose(audiohandle);
...
int32_t CA_CacheAudioChunk (int chunk)
{
    ...
    tcm_flseek(audiohandle,pos,SEEK_SET);
    tcm_fread(audiohandle,audiosegs[chunk],size);
...
//
    ...
    tcm_flseek(grhandle,pos,SEEK_SET);

    if (compressed<=BUFFERSIZE)
    {
        tcm_fread(grhandle,bufferseg,compressed);
        source = bufferseg;
    }
    else
    {
        ...
        tcm_fread(grhandle,source,compressed);
    }
...
//
    ...
    tcm_fread(grhandle,bigbufferseg,compressed);
...
//
    ...
    for (plane = 0; plane<MAPPLANES; plane++)
    {
        ...
        tcm_fread(maphandle,source,compressed);
#ifdef CARMACIZED
        //
