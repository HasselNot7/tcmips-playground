...
int postx;
int postwidth;
uint8_t tcm_halfres = 0;       // render every other column, duplicate pixels
...
void ScalePost()
{
    ...
    int dupmax = tcm_halfres ? 2 : 1;      // half-res: draw each post twice

    for(int dup = 0; dup < dupmax; dup++)
    {
    int px = postx + dup;
    if(px >= (int)viewwidth) break;
    ...
    col = postsource[yw];
#endif
    yendoffs = yendoffs * vbufPitch + px;
...
void AsmRefresh()
{
    ...
    for(pixx=0;pixx<viewwidth;pixx+=1+tcm_halfres)
    {
...
//
    ...
    if (fizzlein)
    {
        // The pre-level flow leaves the palette faded to black; restore it
        // instantly (single step) so the fizzle reveal is visible.
