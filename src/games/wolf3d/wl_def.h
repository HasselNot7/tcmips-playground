...
extern volatile uint8_t tcm_dbg_stage;
extern uint8_t tcm_halfres;
...
#define CHECKMALLOCRESULT(x) if(!(x)) Quit("Out of memory at %s:%i", __FILE__, __LINE__)
...
#else
    static inline char* tcm_itoa(int value, char* string, int radix)
    {
	    (void)radix;
    ...
    static inline char* tcm_ltoa(long value, char* string, int radix)
    {
	    (void)radix;
