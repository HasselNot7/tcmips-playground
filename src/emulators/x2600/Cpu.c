...
while (i--) 
{
  ...
  /* TCMIPS: honour the WSYNC halt - stall instruction execution until the
     current scanline finishes (the crossing that rasterises it), exactly
     like the real TIA. The CPU then resumes during the hblank of the next
     line, so register writes land before visible pixels as on hardware. */
  if (tcm_wsync_halt)
    {
      tcm_wsync_halt = 0;
      if (vbeam_state == DRAWSTATE)
        {
          int line_at_halt = ebeamy;
          while (vbeam_state == DRAWSTATE && ebeamy == line_at_halt)
            do_screen (1);
        }
    }
