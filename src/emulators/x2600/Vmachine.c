...
int tcm_wsync_halt = 0; /* TCMIPS: set by WSYNC; CPU stalls until the line completes */
...
do_hsync (void)
{
  /* TCMIPS: assert the WSYNC halt flag so the CPU stalls until the end of
     the scanline, as on real hardware. Rendering stays in do_screen(). */
  tcm_wsync_halt = 1;
  hbeam_state = HSYNCSTATE;
  ebeamx = -tv_hsync;
  sbeamx = 0;
