...
draw_playfield (void)
{
  ...
  /* TCMIPS fix: replay window per segment now extends to the END of the
     segment (pass end-3; pf_update tests nextx+3 > x). Previously only
     changes landing within the first 3 pixels of a segment were applied,
     so kernels writing PF registers a few cycles into the line (after the
     hblank) lost those writes entirely - e.g. Pong's paddles. */
  pf_update (num, 13, &pfc, pf_max);
  draw_pf0 (&pf[0], 1);
  pf_update (num, 45, &pfc, pf_max);
  draw_pf1 (&pf[0], 1);
  pf_update (num, 77, &pfc, pf_max);
  draw_pf2 (&pf[0], 1);

  pf_update (num, 93, &pfc, pf_max);
  ...
  if (pf[0].ref)
    ...
    {
      draw_pf0 (&pf[0], 1);
      pf_update (num, 125, &pfc, pf_max);
      draw_pf1 (&pf[0], 1);
      pf_update (num, 157, &pfc, pf_max);
      draw_pf2 (&pf[0], 1);
    }
  /* Use last changes */
  for (; pfc < pf_max; pfc++)
